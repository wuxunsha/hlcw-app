              ...
              >
                ...
                <div>
                  ...
                  <p v-if="active === '2'">
                    ...
                    <span>{{
                      (
                        Number(item.payPrice) -
                        Number(item.payPrice) *
                          (Number(costInformation.tradingFee) / 100)
                      ).toFixed(2)
                    }}</span>
                  </p>
                </div>
...
<script>
import { tradingCenterByAccount, feeOrUserInfo, test } from '../../api/index'
export default {
  data() {
    return {
      active: '1',
  ...
  methods: {
    ...
    getTradingCenterByAccount() {
      const data = {
        page: this.queryInfo.pageCurrent,
...
<style>
.transaction-record .van-nav-bar {
  height: 80px;
  background: #415be1;
}
.transaction-record .van-nav-bar .van-nav-bar__title {
  font-size: 36px;
  font-weight: 500;
  color: rgba(255, 255, 255, 1);
}
.transaction-record .van-nav-bar .van-icon {
  color: #fff;
  font-size: 36px;
}
.van-toast {
...
.transaction-record .transaction-box .tab1 .van-pull-refresh {
  overflow: inherit;
}
.transaction-record .transaction-box .tab1 .van-list__finished-text {
  padding: 10px 0;
  font-size: 24px;
}
.transaction-record .transaction-box .tab1 .van-loading__text {
  font-size: 24px;
}
</style>
