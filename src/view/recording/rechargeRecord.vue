<template>
  <div class="withdrawalsRecord">
    <van-nav-bar title="充值记录" fixed :placeholder="true" left-arrow @click-left="onClickLeft" />
    <div class="withdrawalsRecord-box">
      <van-pull-refresh v-model="isRechargeLoading" @refresh="onRechargeRefresh" style="touch-action: pan-y;" class="van-pull-refresh">
        <van-list v-model="rechargeLoading" :finished="rechargeFinished" finished-text="没有更多数据了" @load="rechargeOnLoad" offset="50">
          <ul>
...
export default {
  ...
  methods: {
    // 获取充值记录
    getRechargeList() {
      recharge(this.queryInfo).then(v => {
        if (v.status === 200) {
